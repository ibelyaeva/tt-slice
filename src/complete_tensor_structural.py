...
class TensorCompletionStructural(object):
    ...
    def init_dataset(self, path):
       ...
       #self.mask_indices = self.init_mask()
        self.mask_indices, self.x_mask_img, self.ts = self.init_mask()
        ...
        self.x_init = self.init_mean(self.x_mask_img, self.ts)
        
        self.x_mask_img_data = np.array(self.x_mask_img.get_data())
        self.x_mask_img_truth, self.norm_x_mask_img = tu.normalize_data(self.x_mask_img_data)
        
        # save x_miss_img
        #self.x_miss_img = mt.reconstruct_image_affine(self.ground_truth_img, self.sparse_observation_org)
        #mean_img = math_img("(yt - np.mean(yt))", yt=self.x_miss_img)
        #mean_img_data = np.array(mean_img.get_data())
        #u, s, vh = np.linalg.svd(self.x_temp, full_matrices=True)
        #self.x_init = np.matmul(u * s[..., None, :], vh)
        #self.x_init = copy.deepcopy(mean_img_data)
    ...
    def init_mask(self):
        ...
        self.logger.info("Missing TimePoint Count: " + str(self.time_point_count))
        self.logger.info("Missing Frames Count: " + str(self.frames_count))
        self.logger.info("Start Frame: " + str(self.start_n))
        
        self.x_mask_img, self.random_ts = stp.generate_structural_missing_pattern(self.frames_count, self.start_n, self.target_shape)

        self.x_mask_img_data = np.array(self.x_mask_img.get_data())
        mask_zero_indices_count = np.count_nonzero(self.x_mask_img_data==0)
        self.logger.info("Zero Count 1: " + str(mask_zero_indices_count))
        
        self.mask_indices = np.ones_like(self.x_true_reshaped)
        self.mask_indices[self.x_mask_img_data == 0] = 0
        
        mask_zero_indices_count2 = np.count_nonzero(self.mask_indices==0)
        self.logger.info("Zero Count 2: " + str(mask_zero_indices_count2))
                                                                             
        return self.mask_indices, self.x_mask_img, self.random_ts
